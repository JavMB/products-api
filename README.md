...
Esta API implementa una arquitectura hexagonal (Ports & Adapters) combinada con el patrón Mediator. La decisión de usar
...
graph TD
    ...
    classDef infrastructure fill:#1976d2,stroke:#0d47a1,stroke-width:2px,color:#ffffff
    classDef mediator fill:#7b1fa2,stroke:#4a148c,stroke-width:2px,color:#ffffff
    classDef application fill:#388e3c,stroke:#1b5e20,stroke-width:2px,color:#ffffff
    classDef domain fill:#f57c00,stroke:#e65100,stroke-width:2px,color:#ffffff
...
Los DTOs nos permiten tener control total sobre la API sin exponer directamente nuestro modelo interno. Spring usa
Jackson para convertir automáticamente el JSON entrante a un objeto Java cuando encuentra `@RequestBody`. Sin esta
anotación, tendríamos que manejar manualmente la deserialización.
...
El mediator actúa como un despachador central. En lugar de que los controllers conozcan directamente los handlers, el
mediator se encarga de encontrar y ejecutar el handler apropiado. Esto hace que agregar nuevas funcionalidades sea más
simple - solo necesitas crear el handler y automáticamente queda registrado.
...
Cada handler maneja un caso de uso específico. Esto hace que el código sea más fácil de testear y mantener, ya que cada
operación está claramente separada.
...
El dominio contiene las entidades de negocio sin dependencias externas. Puedes testear estas clases sin necesidad de
Spring o cualquier framework.
...
**Facilita el testing**
El dominio se puede testear de forma aislada, y cada handler puede testearse independientemente. Los tests de
integración se enfocan únicamente en verificar que las capas se comunican correctamente.

**Independencia de frameworks**
Si mañana quisiéramos cambiar Spring por otro framework, solo tendríamos que modificar la capa de infrastructure. El
dominio y la aplicación permanecerían intactos.
...
MapStruct genera automáticamente el código de mapeo en tiempo de compilación. Es más rápido que usar reflection y más
seguro que el mapeo manual. El `componentModel = "spring"` hace que el mapper sea un componente de Spring, permitiendo
...
La complejidad inicial de esta arquitectura se compensa con la facilidad de mantenimiento y testing que proporciona a
